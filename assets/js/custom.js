...
if ($('.side-menu').length) {
	...
	//Dropdown Button
	$('.side-menu li.dropdown .dropdown-btn').on(
		'click',
		function () {
			$(this).toggleClass('open');
			$(this).prev('ul').slideToggle(500);
	...
	//Menu Toggle Btn
	$('.main-header .mobile-nav-toggler-two').on(
		'click',
		function () {
	...
	//Menu Overlay
	$('.main-header .side-menu-overlay').on(
		'click',
		function () {
...
function bodylayout() {
	if ($('.boxed_switch_menu').length) {
		$('.body_switch_btn button').on(
			'click',
			function () {
				$('.body_switcher').toggleClass(
					'switcher-show'
				);
			}
		);
		...
		$('.accordion-box').on(
			...
			function () {
				if ($(this).hasClass('active') !== true) {
					$('.accordion .accord-btn').removeClass(
						'active'
					);
				}
...
function beforeAfterTwentyTwenty() {
	if ($('.before-after-twentytwenty').length) {
		$('.before-after-twentytwenty').each(
			function () {
				var Self = $(this);
...
//Progress Bar / Levels
if (
	$('.progress-levels .progress-box .bar-fill')
		.length
) {
